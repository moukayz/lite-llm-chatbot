...
describe('useChat', () => {
  ...
  test('streams updates and toggles isStreaming', async () => {
    ...
    const { result } = renderHook(() =>
      useChat({ onStreamUpdate, onError })
    );
    ...
    expect(onStreamUpdate).toHaveBeenCalledWith({ text: 'Thinking... ', type: 'thinking', isDone: false });
    // Then answer chunks accumulate
    expect(onStreamUpdate).toHaveBeenCalledWith({ text: 'Hello', type: 'answer', isDone: false });
    expect(onStreamUpdate).toHaveBeenCalledWith({ text: 'Hello World', type: 'answer', isDone: false });
    // Final done signal
    expect(onStreamUpdate).toHaveBeenCalledWith({ text: '', type: 'answer', isDone: true });
