...
export const updateChatSession = async (
  ...
  try {
    ...
    console.error(
      `Failed to update chat session ${sessionId} in localStorage:`,
      error,
    );
    throw error;
  }
...
export const fetchChatSessions = async (): Promise<ChatSession[]> => {
  try {
    ...
    const sessions = JSON.parse(sessionsData) as Array<
      Omit<ChatSession, "createdAt" | "updatedAt"> & {
        createdAt: string;
        updatedAt: string;
      }
    >;
...
export const fetchChatSessionMessages = async (
  ...
  try {
    ...
    console.error(
      `Failed to fetch messages for chat session ${sessionId} from localStorage:`,
      error,
    );
    return [];
  }
